...
const SolicitacaoSchema = Yup.object().shape({
  servico_id: Yup.string().required('É preciso selecionar um serviço'),
  agricultor_id: Yup.string().required('É preciso selecionar um agricultor'),
  propriedade_id: Yup.string().required('É preciso selecionar uma propriedade'),
...
export default function SolicitacoesPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [solicitacoesRes, agricultoresRes, servicosRes, propriedadesRes] = await Promise.all([
        getSolicitacoes(),
        getAgricultores(),
        getServicos(),
        getPropriedades()
    ...
    <Box pt={{ base: "130px", md: "80px", xl: "80px" }}>
      <Card>
        <Flex justify='space-between' align='center' mb='20px'>
          <Text fontSize='2xl' fontWeight='700' color={textColor}>Lista de Solicitações</Text>
          <Button colorScheme='brand' onClick={() => handleOpenForm()}>Nova Solicitação</Button>
        </Flex>
            ...
            {solicitacoes.map((sol) => {
              ...
              return (
                <Tr key={sol.id}>
                  ...
                  <Td>{propriedade ? propriedade.terreno : 'N/A'}</Td>
      ...
      <Formik
        initialValues={solicitacaoSelecionada || { servico_id: '', agricultor_id: '', propriedade_id: '', status: 'Pendente' }}
              ...
              <Form>
                ...
                <ModalBody>
                  <Field name='servico_id'>{({ field, form }) => (
                    <FormControl isInvalid={form.errors.servico_id && form.touched.servico_id}>
                      ...
                      <FormErrorMessage>{form.errors.servico_id}</FormErrorMessage>
                    </FormControl>
                  )}</Field>
                  <Field name='agricultor_id'>{({ field, form }) => (
                    <FormControl mt={4} isInvalid={form.errors.agricultor_id && form.touched.agricultor_id}>
                      ...
                      <Select {...field} placeholder="Selecione o agricultor" onChange={(e) => {
                        props.setFieldValue('agricultor_id', e.target.value);
                        props.setFieldValue('propriedade_id', '');
                      }}>
                        {agricultores.map(ag => (
                  ...
                  <Field name='propriedade_id'>{({ field, form }) => (
                    <FormControl mt={4} isInvalid={form.errors.propriedade_id && form.touched.propriedade_id}>
                      <FormLabel>Propriedade</FormLabel>
                      <Select {...field} placeholder="Selecione a propriedade" disabled={!props.values.agricultor_id}>
                        {propriedadesDoAgricultor.map(prop => (
                          <option key={prop.id} value={prop.id}>{prop.terreno}</option>
                        ))}
                      </Select>
                      <FormErrorMessage>{form.errors.propriedade_id}</FormErrorMessage>
                    </FormControl>
                  )}</Field>
